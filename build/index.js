...
class JuliaDocServer {
    ...
    constructor() {
        ...
        this.cache = new Cache(300); // 5 minute cache
        // Register tools
        this.setupTools();
    }
    async runJuliaCommand(code) {
        try {
    ...
    setupTools() {
        // Get documentation
        this.server.tool("get-doc", "Get Julia documentation for a package, module, type, function, or method", {
            ...
            try {
                const doc = await this.runJuliaCommand(`println(@doc ${path})`);
        ...
        // Get source code
        this.server.tool("get-source", "Get Julia source code for a function, type, or method", {
            path: z.string().describe("Path to Julia object (e.g., 'Base.sort', 'AbstractArray')"),
        }, async ({ path }) => {
